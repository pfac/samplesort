...
int main ( int argc , char *argv[] )
{
	...
	// 		cerr << getpid() << "\tProcess " << thread << " of " << threads << '/' << threads << endl;
	// 	}
	// }

	LArray keys( argv[1] );
	long unsigned threads;		//<	Number of threads to ask the parallel zone.
	long unsigned partitions;		//<	Number of partitions which will compose the array to sort.
	
	partitions = strtoul(argv[2], NULL, 0);
	...
	long unsigned parts_lst = partitions - 1;
	...
	pfirsts[0] = 0;
	for (long unsigned partition = 1 ; partition < partitions ; ++partition )
		pfirsts[partition] = pfirsts[partition-1] + psizes[partition-1];


	//	LOCAL SORT
	#pragma omp parallel for num_threads(threads) schedule(static)
	for (long unsigned partition = 0; partition < partitions; ++partition)
	{
		//int thread = omp_get_thread_num();
		// int threads = omp_get_num_threads();
		// #pragma omp critical
		// {
		// 	cerr << "Thread " << thread << " of " << threads << '/' << threads << endl << '\t';
	...
	// {
		...
		diveven (psizes[partition], partitions, samp_ints + partition * partitions);
			...
			// 		cerr << samp_ints[thread * threads + n] << ' ';
			// 	cerr << endl;	
			// }
		long unsigned w = 0;
	...
	//	FIND PIVOTS
	//diveven (procs_sqr, threads, piv_ints);
		//debug
	...
	for (long unsigned partition = 0; partition < partitions; ++partition)
	{
		...
		for (long unsigned p = 0; p < partitions; ++p)
			...
			// #pragma omp critical
			// {
			// 	cerr << "Thread " << thread << " of " << threads << '/' << threads << endl << '\t' << psizes[thread] << endl;
			// 	cerr << endl;	
			// }
	}
	...
	for (long unsigned p = 1; p < partitions; ++p)
		...
		// for (unsigned long r = 0; r < threads; ++r)
