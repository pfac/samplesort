...
struct TBB_LocalSort {
	...
	void operator() (const tbb::blocked_range<long unsigned> &range) const {
		int p = range.begin();
		qsort(data + firsts[p], sizes[p], sizeof(long), comparel);
	}
};
...
int main ( int argc , char *argv[] )
{
	...
	// 		// cerr << endl;
	// }
	TBB_LocalSort localsort;
	localsort.data = keys.data;
	localsort.firsts = pfirsts;
	localsort.sizes = psizes;
	parallel_for(tbb::blocked_range<long unsigned>(0, processors, 1), localsort);
